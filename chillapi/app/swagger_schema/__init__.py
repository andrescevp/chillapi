...
def abort(http_status_code, schema=None, **kwargs):
    """
    Proxy method to throw a non authorized request
...
class Api(restful_Api):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._swagger_object = {
            "openapi": "3.0.0",
            "info": {"title": "", "description": "", "termsOfService": "", "contact": {}, "license": {}, "version": "0.0"},
        ...
        if add_api_spec_resource:
            ...
            self.add_resource(
                create_swagger_endpoint(self.get_swagger_doc(), _security_level=self._security_level), *api_spec_urls, endpoint="swagger"
            )

    def add_resource(self, resource: Resource, *urls, **kwargs):
        """
        Adds a Flask Resource to the API
...
class _BaseExtractorImpl(Extractor):
    ...
    def _extract_model(self, obj, definitions):
        ...
        if not issubclass(obj, Schema):
            raise ValueError(f'"{obj}" is not a subclass of the schema model')
...
class _RequestParserExtractorImpl(_BaseExtractorImpl):
    ...
    def _get_reqparse_args(self, operation):
        ...
        if make_model:
            ...
            params.append({"name": "body", "description": "Request body", "in": "query", "schema": model, "required": model.is_required()})
    ...
    def _reqparser_arg_to_swagger_param(cls, arg):
        ...
        """
        param = {"name": arg.name, "description": arg.help, "required": arg.required}
        if arg.choices:
        ...
        if self.properties:
            for k, v in kwargs.items():
                if k not in self.properties:
                    raise ValueError(f'The model "{self.__class__.__name__}" does not have an attribute "{k}"')
                if "type" in self.properties[k]:
                    type_ = self.properties[k]["type"]
                    if type_ == "integer" and not isinstance(v, int):
                        raise ValueError('The attribute "{}" must be an int, but was "{}"'.format(k, type(v)))
                    if type_ == "number" and not isinstance(v, int) and not isinstance(v, float):
                        raise ValueError('The attribute "{}" must be an int or float, but was "{}"'.format(k, type(v)))
                    if type_ == "string" and not isinstance(v, str):
                        raise ValueError('The attribute "{}" must be a string, but was "{}"'.format(k, type(v)))
        ...
        if hasattr(self, "required"):
            for key in self.required:
                if key not in kwargs:
                    raise ValueError(f'The attribute "{key}" is required')
...
def get_swagger_blueprint(docs, api_spec_url="/api/swagger", **kwargs):
    ...
    api.add_resource(create_swagger_endpoint(swagger_object), *api_spec_urls, endpoint="swagger")
