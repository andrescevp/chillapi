from flask_restful_swagger_3 import Schema as SwaggerSchema, Schema
...
def get_filter_schema(class_name) -> SwaggerSchema:
...
def get_size_schema(class_name):
    class SizeListModel(SwaggerSchema):
        type = 'object'
        properties = {
            'limit': {
                'type': 'integer',
...
def columns_map_to_swagger_properties(
        columns_map,
        columns_swagger_definition: dict = None
):
    properties = {}
    ...
    for property_name, column_info in columns_map.items():
        ...
        if columns_swagger_definition:
            properties[property_name] = {
                **properties[property_name],
                **columns_swagger_definition[property_name].column_swagger_properties
            }
    return properties


def get_response_swagger_schema(
        columns_map: dict,
        class_name: str,
        columns_swagger_definition: dict = None
):
...
def get_form_array_swagger_schema(
        class_name: str,
        form_schema: type(Schema),
        class_name_postfix : str = 'ArrayFormModel',
        min_items : int = 1,
        max_items : int = 100
):
    class ArrayFormModel(SwaggerSchema):
        type = 'array'
        items = form_schema
        minItems = min_items
        maxItems = max_items
...
def get_list_filtered_response_swagger_schema(
        columns_map: dict,
        request_schema: dict,
        class_name: str,
        columns_swagger_definition: dict = None
...
def get_list_filtered_request_swagger_schema(
        class_name: str,
        columns_map: dict
):
    schema = []
    for property_name, column_info in columns_map.items():
        schema.append({
            ...
            'schema': get_filter_schema(class_name)
        })
    ...
    schema.append({
        ...
        'allowEmptyValue': True,
        'required': False,
        'schema': get_order_schema(class_name)
    })
    ...
    schema.append({
        ...
        'allowEmptyValue': True,
        'required': False,
        'schema': get_size_schema(class_name)
    })
...
def get_revisable_response_swagger_schema():
    class RevisableOperationResponseModel(SwaggerSchema):
        type = 'object'
        properties = {
            'message': {
                'type': 'string',
...
def get_error_swagger_schema():
    class ErrorResponseModel(SwaggerSchema):
        type = 'object'
        properties = {
            'code': {
                'type': 'integer',
...
def get_not_found_swagger_schema():
    class NotFoundResponseModel(SwaggerSchema):
        type = 'object'
        properties = {
            'message': {
                'type': 'string',
...
def get_request_swagger_schema(
        columns_map: dict,
        class_name: str,
        columns_swagger_definition: dict = None,
