...
from chillapi.swagger.utils import get_revisable_response_swagger_schema, get_error_swagger_schema, \
        ...
        'parameters': [
            {
                ...
                'in': 'path',
                'schema': {
                    'type': 'integer' if id_field_where_type == 'int:' else 'string'
                }
            }
        ],
        'responses': {
            ...
            '404': {
                'description': f'Not found response model',
                'content': {
                    'application/json': {
                        'schema': not_found_swagger_schema
                    }
                }
            },
            '500': {
                'description': f'Operation fail',
                'content': {
                    'application/json': {
                        'schema': error_swagger_schema
                    }
                }
            }
        }
...
def get_delete_single_endpoint_schema(class_name, id_field_where_type):
    return {
        ...
        'responses': {
            '200': {
                'description': f'Operation success',
                'content': {
                    'application/json': {
                        'schema': revisable_response_swagger_schema
                    }
                }
            },
            '400': {
...
def get_get_list_endpoint_schema(class_name, response_schema, request_schema):
    return {
        ...
        'responses': {
            '200': {
                'description': f'{class_name} list response model',
                'content': {
                    'application/json': {
                        'schema': response_schema
                    }
...
def get_put_list_endpoint_schema(class_name, form_schema_model):
    return {
        ...
        'requestBody': {
            ...
            'required': True,
            'content': {
                'application/json': {'schema': form_schema_model}
            }
        },
        'responses': {
            '200': {
...
def get_query_endpoint_schema(class_name: str, tags: list, query_parameters: List,
    ...
    schema = {
        ...
        'responses': {
            '200': {
                'description': f'{class_name} response model',
                'content': {
                    'application/json': {'schema': response_schema} if response_schema else {
                        'type': 'object'
                    }
                }
            },
...
def get_delete_list_endpoint_schema(class_name, request_body_schema):
    return {
        ...
        'requestBody': {
            ...
            'required': True,
            'content': {
                'application/json': {'schema': request_body_schema}
            }
        },
        'responses': {
            '200': {
                'description': f'{class_name} response model',
                'content': {
                    'application/json': {'schema': {'type': 'string', 'example': 'ok'}}
                }
            },
            '400': {
                'description': f'Operation fail',
