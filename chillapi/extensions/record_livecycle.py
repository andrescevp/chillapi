...
class SoftDeleteExtension(TableExtension):
        ...
        if "cascade" in self.config:
            ...
            if "one_to_many" in _cascades:
                ...
                for _otm, _relation in enumerate(_one_to_many):
                    ...
                    one_to_many(
                        _relation_table=_relation_table, _pk=_pk, _fk=_fk, _inspector=self.inspector, _extension_field=self.config["default_field"]
                    )
            ...
            if "many_to_many" in _cascades:
                ...
                for _otm, _relation in enumerate(_one_to_many):
                    ...
                    many_to_many(
                        _relation_table=_relation_table,
                        _pk=_pk,
                        _relation_join_table=_relation_join_table,
                        _relation_columns=_relation_columns,
    ...
    def validate(self):
        ...
        def many_to_many(**args):
            ...
            if _relation_columns["main"] not in _relation_join_table_columns:
                raise ConfigError(f'join_columns[main]: {_relation_columns["main"]} in Table {_relation_join_table} does not exists!')

            if _relation_columns["join"] not in _relation_join_table_columns:
                raise ConfigError(f'join_columns[join]: {_relation_columns["join"]} in Table {_relation_join_table} does not exists!')
    ...
    def soft_delete(self, id_field, id, response):
        ...
        def one_to_many(**args):
            ...
            _relation_ids = _repository.fetch_by(_relation_table, [_pk], {_fk: {"op": "=", "value": id}}, {_fk: id})
        ...
        def many_to_many(**args):
            ...
            _repository = args["_repository"]
            _extension_field = args["_extension_field"]
            sql = create_select_join_soft_delete_filter(_relation_table, _pk, _relation_join_table, _relation_columns)
        ...
        _field = self.config["default_field"]
        self.repository.update_record(self.table, id_field, id, {id_field: id, _field: now})
    ...
    def soft_delete_batch(self, table_name, extension_field, id_field, data):
        ...
        def one_to_many(**args):
            ...
            _cascade_ids = []
            for _fk_id in data:
                _relation_ids = self.repository.fetch_by(_relation_table, [_pk], {_fk: {"op": "=", "value": _fk_id}}, {_fk: _fk_id})
                _cascade_ids += [x[0] for x in _relation_ids.fetchall()]
