...
def ChillApi(app: Flask = None, config_file: str = _CONFIG_FILE, export_path: str = CWD):
    ...
    api = Api(app,
              ...
              externalDocs = api_config['app']['externalDocs'] if 'externalDocs' in api_config[
                  'app'] else None,
              components = {
                      'securitySchemes': api_config['app']['securitySchemes'] if 'securitySchemes' in
                                                                                 api_config[
                                                                                     'app'] else None
                      }
              )
    ...
    if 'securitySchemes' in api_config['app'] and len(
            api_config['app']['securitySchemes'].keys()) > 0:
    ...
    simplejson.dump(api.get_swagger_doc(), open(f'{export_path}/{_app_name}_swagger.json', 'w'),
                    indent = 2,
                    cls = CustomEncoder,
                    for_json = True)
    ...
    simplejson.dump(config.to_dict(), open(f'{export_path}/{_app_name}_api.config.json', 'w'),
                    indent = 2,
                    cls = CustomEncoder,
                    for_json = True)
